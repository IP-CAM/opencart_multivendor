...
class ControllerInstallStep2 extends Controller {
	...
	public function index() {
		...
		//seller config
		if(!is_file(DIR_OPENCART.'seller/config')){
		...
		$data['seller_config'] = DIR_OPENCART . 'seller/config.php'; //seller, my hack
	...
	private function validate() {
		...
		if (!file_exists(DIR_OPENCART . 'seller/config.php')) {
			$this->error['warning'] = $this->language->get('error_seller_exist');
		} elseif (!is_writable(DIR_OPENCART . 'seller/config.php')) {
			$this->error['warning'] = $this->language->get('error_seller_writable');
		}
